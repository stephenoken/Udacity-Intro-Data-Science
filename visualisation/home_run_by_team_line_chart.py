...
def lineplot_compare(hr_by_team_year_sf_la_csv):
    ...
    # in this exercise.
    home_runs_df = pandas.read_csv(hr_by_team_year_sf_la_csv)
    gg = ggplot(home_runs_df, aes(x='yearID', y='HR', color="teamID")) + geom_line()
    gg = gg + ggtitle('Total HRs by year') + xlab('Year') + ylab('Home Runs')
    return gg


if __name__ == "__main__":
    lineplot_compare("./hr_by_team_year_sf_la.csv").show()
